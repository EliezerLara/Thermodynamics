...
int main(){
	...
	double h=0.01, ua=1.0, ub, r=10.0, k, c=1.0, t=0.2;
	double u0[NMAX];
	int i,n,j,m;
...
for(j=0;j<=m; j++){
	...
	b[0]=(1-2*r)*u0[0]+r*u0[1]+r*ua;
	b[n-1]=(1-2*r)*u0[n-1]+r*u0[n-2]+r*ub;

	for(i=1; i<n-1; i++){
		b[i]=(1-2*r)*u0[i]+r*(u0[i-1]+u0[i+1]);
	}
